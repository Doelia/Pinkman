package protos.communication.messages;
...
public class TargetMessageWrapper {
	...
	public static void wrapAndSendMessageToAgentType(String message,
			WarBrain brain, CoordPolar cp, WarAgentType type)
	{
		CoordCartesian c2 = cp.toCartesian();
		String sX = new String("" + c2.getX());
		String sY = new String("" + c2.getY());
		brain.broadcastMessageToAgentType(type, message, sX,sY);
	}

	public static void wrapAndSendMessageToAgentType(String informFoundEnemy,
			WarBrain brain, WarPercept wp, WarAgentType warbase,
			WarAgentType... warturret) {
		wrapAndSendMessageToAgentType(informFoundEnemy,brain,wp,warbase);
		for(WarAgentType a : warturret)
		{
			wrapAndSendMessageToAgentType(informFoundEnemy,brain,wp,a);
		}
	}
}
